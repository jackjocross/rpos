      ...
      if (action == '/web/snapshot.jpg') {
        try {
          if (this.ffmpeg_process != null) {
            ...
            var cmd = `ffmpeg -fflags nobuffer -probesize 42M -rtsp_transport tcp -i rtsp://127.0.0.1:${this.config.RTSPPort}/${this.config.RTSPName} -vframes 1  -r 1 -s 640x360 -y /dev/shm/snapshot.jpg`;
